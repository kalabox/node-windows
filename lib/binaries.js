...
module.exports = {
  ...
  elevate: function(cmd,options,callback) {
    ...
    // Make sure we've written our elevate stuff out to ~/.kalabox/downloads
    // This is a kalabox specific hack to deal with the fact that
    // our referenced files cannot exist inside of the binary
    // However we are going to do this regardless of whether we are
    // in a binary or not so its easier to test.
    var kalaboxRoot = path.resolve(__dirname, '..', '..', '..');
    var confPath = path.join(kalaboxRoot, 'lib', 'core', 'config.js');

    var fs = require('fs');
    var elePath;

    // Check to make sure we can load kalabox config, otherwise proceed
    // normally
    if (fs.existsSync(confPath)) {

      // Since we have kalabox we can assume fs-extras exists
      // and config.js exists
      var fsExtra = require(path.join(kalaboxRoot, 'node_modules', 'fs-extra'));
      var conf = require(confPath);

      // Get relevant paths
      var sysRoot = conf.getGlobalConfig().sysConfRoot;
      var downloadPath = path.join(sysRoot, 'downloads');
      // Make sure downloads path exists
      fsExtra.mkdirpSync(downloadPath);

      // Make sure our files exist and create if not
      var elevators = ['elevate.cmd', 'elevate.vbs'];
      elevators.forEach(function(value, key) {
        var source = path.join(bin, 'elevate', value);
        var dest = path.join(downloadPath, value);
        // @todo: error handling
        if (!fs.existsSync(dest)) {
          fsExtra.copySync(source, dest, {clobber: true});
        }
      });

      // Return exported file as source
      elePath = path.join(downloadPath, 'elevate.cmd');
